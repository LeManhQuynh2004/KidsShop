...
public class Shipment_Payment_Adapter extends BaseAdapter {

    Context context;
    ArrayList<Shipment> list;

    public Shipment_Payment_Adapter(Context context, ArrayList<Shipment> list) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        view.setOnClickListener(view1 -> {
            ((MainActivity)context).getSupportFragmentManager().beginTransaction().replace(
                    R.id.fragment_container, new Shipment_Fragment()).commit();
        });
        return view;
    }
}
